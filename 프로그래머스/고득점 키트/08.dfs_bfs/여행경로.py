def solution(tickets):
    ...
    # 딕셔너리로 변환
    for start, end in sorted(tickets, key=lambda item : item[1], reverse=True):
...
def dfs(ticket_dict, start, path):
    print(start, ticket_dict)
    if ticket_dict.get(start):
        next_ticket = ticket_dict[start].pop()
    ...
    # 잘못된 길로 왔을 떄
        ...
        if ticket_dict.get(path[-2]):
            ticket_dict[path[-2]].append(path.pop())
        else:
            ticket_dict[path[-2]] = [path.pop()]
        # 여기 살펴 봐야됨
